...
public class WalletRepository {
    ...
    private static final Sql2o SQL_DB = new Sql2o("jdbc:postgresql://localhost:5434/iron_sheep_wallet", "iron_sheep", "iron_sheep");

    public Wallet getWallet(String id) {
        List<Wallet> wallets;
        try (Connection con = SQL_DB.open()) {
            wallets = con.createQuery("SELECT * FROM iron_sheep_wallet.wallet WHERE id = :id")
                    .addParameter("id", id)
    ...
    private void createWallet(Wallet wallet) {
        try (Connection con = SQL_DB.open()) {
            con.createQuery("INSERT INTO iron_sheep_wallet.wallet(id, balance) VALUES (:id, :balance)")
    ...
    private void updateWallet(Wallet wallet) {
        try (Connection con = SQL_DB.open()) {
            con.createQuery("UPDATE iron_sheep_wallet.wallet SET balance = :balance WHERE id = :id")
                    .addParameter("id", wallet.getId())
                    .addParameter("balance", wallet.getBalance())
                    .executeUpdate();
        }
